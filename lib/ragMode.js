 ...
 *   - `@rag:keyword`   -> force a keyword-only fast-path search (zero ONNX/FlashRank).
 *   - `@rag:exact`     -> alias of `@rag:keyword` (BM25 has no phrase operator).
 *
 * @module lib/ragMode
 */
...
const TAG_RE = /^\s*@rag(?::(keyword|exact))?(?=\s|$)/i;

/**
 * Strip a leading `@rag` / `@rag:keyword` / `@rag:exact` tag from user text.
 * @param {string} text - Raw user message.
 * @returns {{text: string, tag: string|null}} Cleaned text + detected tag
 *   ('manual' | 'keyword' | 'exact' | null).
 */
...
export function resolveRagSearch({ ragMode = 'auto', tag = null } = {}) {
  ...
  const tagged = tag === 'keyword' || tag === 'exact' || tag === 'manual';

  // Explicit tag => deterministic, forced execution.
  if (tagged) {
    const search_mode = tag === 'keyword' || tag === 'exact' ? 'keyword' : 'hybrid';
