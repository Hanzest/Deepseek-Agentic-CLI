...
// ---------------------------------------------------------------------------
// copyActiveToHistory(taskName)
// Copies all files from artifacts/active/ into:
//   artifacts/history/{taskName}/{YYYY-MM-DD_HH.MM.SS}/
// Unlike archiveActiveToHistory, this uses copy so files remain in active/.
// If active/ is empty or does not exist, no-op.
// Returns the archive destination path (or null if no-op).
// ---------------------------------------------------------------------------
export function copyActiveToHistory(taskName) {
    const activePath = resolveActive();
    ...
    // Recursively copy all contents from active to archive (preserving original)
    try {
        fs.cpSync(activePath, archiveDir, { recursive: true });
    } catch (err) {
        console.error(`[artifactManager] Failed to copy artifacts to history: ${err.message}`);
        return null;
    }
...
// Checks content of plan file against required headers.
// ---------------------------------------------------------------------------
export function validatePlanContent(content) {
    ...
    for (const sec of requiredSections) {
        ...
        const regex = new RegExp(`^#+\\s*.*${escapedSec}`, 'im');
