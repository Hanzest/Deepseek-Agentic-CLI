...
const HYPERPARAMETERS = {
    ...
    system_prompt:
        `
            ...
            3. **Tertiary:** Domain-specific \`SKILL.md\` files in \`${SKILLS_DIR}\`
            ...
            2. Scan \`${SKILLS_DIR}\` for relevant SKILL.md files using these specific tool steps:
               a. **Discover** — Call \`get_project_tree("${SKILLS_DIR}", 2)\` to list all skill directories.
               b. **Match** — Call \`multi_file_search_string\` with \`root_path: "${SKILLS_DIR}"\` and \`glob_pattern: "**/*.md"\` 
...
async function multiTurnLoop(model_name) {
    ...
    while (!stop) {
        ...
        const modeLabel = SessionContext.agentMode === "plan" ? "Plan" : "Agent";
        let prefixedInput = `You are in ${modeLabel} Mode. ${user_input}`;
        if (!SessionContext.workDirInjected) {
            prefixedInput = `You are in ${modeLabel} Mode. Working directory: ${USER_WORKING_DIR}. ${user_input}`;
