...
import { ask, askYesNo, startChat, thinkingToggle, createPromptLoop, askSearchKeyword } from "./cliInput.js";
...
export { SessionContext, MUTATION_BLOCKED_TOOLS, PRICING, getAuditData, getSessionMemoryContent, getActiveMessages, printPerToolAudit };
...
const HYPERPARAMETERS = {
    ...
    system_prompt:
        `
            ...
            to the codebase (less than 5-10 files created/modified/deleted depending on complexity).
            ...
            3. **Tertiary:** Domain-specific \`SKILL.md\` files in \`docs/skills/\`

            ---

            ## Tools Usage
            ### 1.**Batch-first mandate:** Must leverage batch tool-calling to execute tools
            ...
            ### 2.**Similar-tool disambiguation:**
              - **Exploration:** always prefer \`get_project_tree\` over \`execute_terminal_command\`.
              ...
              - **File Reading:** use \`read_file_chunk\` when the path is known. Use \`multi_file_search_string\` to locate a specific string/pattern lives.

            ---

            ## Mandatory Planning Pipeline
            ### Step 1: Context & Skill Gathering (Execute First)
            Before prompting the user or drafting a plan, you must gather baseline context:
            1. Identify high-level codebase constraints (tech stack, dependencies, basic project structure).
            2. Scan \`docs/skills/\` for any \`SKILL.md\` matching the request's domain. If a match is found,
            you **MUST** read it immediately to extract domain constraints, principles, and anti-patterns.
            ...
            *Do not make blind assumptions. Use the context gathered in Step 1 to evaluate your next move:*
            ...
            ### Step 3 - Conduct Implementation Plan (Plan Mode Only)
                **Plan Template**
                ...
                saved as an artifact in the \`artifacts/active/\` folder and include the following sections:
...
function printPerToolAudit(messages, model_name) {
    ...
    for (const name of toolNames) {
        ...
        const calls = String(m.calls).padStart(7);
        const inp = m.inputTokens.toLocaleString().padStart(14);
        const out = m.outputTokens.toLocaleString().padStart(14);
        const total = (m.inputTokens + m.outputTokens).toLocaleString().padStart(14);
...
function getUserPrompt() {
    if (SessionContext.firstTurn) {
        ...
        return colorize("Enter your message (type /help for commands, 'exit' or '/exit' to quit):\n", C.user);
    }
...
function printLoadedMessages(messages) {
    ...
    for (const msg of messages) {
        if (msg.role === "user") {
            const cleanContent = msg.content.replace(/^You are in (Plan|Agent) Mode\.\s*/i, "");
...
async function multiTurnLoop(model_name) {
    ...
    while (!stop) {
        ...
        const commandResult = handleSlashCommand(trimmedInput, model_name, messages, token_estimates, thinking, iterationCalls);
        ...
        if (commandResult === "load") {
            ...
            for (const filePath of files) {
                try {
                    ...
                    if (!title) {
                        ...
                        if (firstUser) {
                            title = firstUser.content.replace(/^You are in (Plan|Agent) Mode\.\s*/i, "");
            ...
            while (!loadedSuccessfully) {
                ...
                console.log(colorize(`  ${titlePrefix} (Page ${currentPage} of ${totalPages}):`, C.heading));
                console.log(`  [1] ${colorize("Search conversations", C.tool)}`);

                pageItems.forEach((convo, index) => {
                    ...
                    console.log(`  [${index + 2}] ${convo.title}`);
                    console.log(colorize(`      Latest: ${dateStr} | Messages: ${latest.messageCount}${verStr}`, C.system));
                });
                console.log("");

                let paginationPrompt = "";
                const validOptions = [];
                validOptions.push("1");
                for (let i = 0; i < pageItems.length; i++) {
                    validOptions.push(String(i + 2));
                }

                if (currentPage < totalPages) {
                    console.log(`  [N] Next Page`);
                    paginationPrompt += ", (N)ext Page";
                    validOptions.push("n");
                }
                if (currentPage > 1) {
                    console.log(`  [P] Previous Page`);
                    paginationPrompt += ", (P)rev Page";
                    validOptions.push("p");
                }
                console.log(`  [Enter/C] Cancel`);
                console.log("");

                const choice = await ask(
                    colorize(`  Select an option (1-${pageItems.length + 1}${paginationPrompt}) or press Enter to cancel: `, C.consent)
                );

                const cleanChoice = choice.trim().toLowerCase();

                if (cleanChoice === "" || cleanChoice === "c") {
                    console.log(colorize("  Load cancelled.\n", C.dim));
                    break;
                }

                if (!validOptions.includes(cleanChoice)) {
                    console.log(colorize("  Invalid option. Please choose from the list.\n", C.warning));
                    continue;
                }
                ...
                // Selection from list: option index is num - 2
                const selectedConvo = pageItems[num - 2];
                ...
                if (selectedConvo.versionsCount > 1) {
                    ...
                    while (!versionSelected) {
                        console.log("");
                        console.log(colorize(`  Select a version of "${selectedConvo.title}" to load:`, C.heading));
                        selectedConvo.versions.forEach((ver, vIndex) => {
                            const label = vIndex === 0 ? " (Latest)" : "";
                            console.log(`  [${vIndex + 1}] ${ver.savedAt.toLocaleString()} - ${ver.messageCount} messages${label}`);
                        });
                        console.log(`  [C] Cancel version selection`);
                        console.log("");

                        const verChoice = await ask(
                            colorize(`  Select version (1-${selectedConvo.versionsCount}) [1]: `, C.consent)
                        );

                        const cleanVerChoice = verChoice.trim().toLowerCase();
                        if (cleanVerChoice === "c") {
                            chosenVersion = null;
                            versionSelected = true;
                        } else if (cleanVerChoice === "") {
                            chosenVersion = selectedConvo.latestVersion;
                            versionSelected = true;
                        } else {
                            const vNum = parseInt(cleanVerChoice, 10);
                            if (!isNaN(vNum) && vNum >= 1 && vNum <= selectedConvo.versionsCount) {
                                chosenVersion = selectedConvo.versions[vNum - 1];
                                versionSelected = true;
                            } else {
                                console.log(colorize("  Invalid version choice.", C.warning));
                            }
                        }
                    }
                }
                ...
                if (chosenVersion === null) {
                    continue; // return to conversation selection list
                }
