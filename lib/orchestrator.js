...
// ---------------------------------------------------------------------------
// Session Banner — prints a clean summary after startup
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// /status handler — shows full session info
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Mode switch banner (replaces console.clear)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Threshold-gated compact token line
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Iteration summary (gated behind verbose)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Prompt for auto-save preference at startup
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Prompt string helper (short after first turn)
// ---------------------------------------------------------------------------
...
async function multiTurnLoop(model_name) {
    ...
    // Use persistent prompt loop with history
    const { ask: askWithHistory, addToHistory, close: closePrompt } = createPromptLoop();
    ...
    while (!stop) {
        ...
        // Replace per-turn token dump with threshold-gated compact line
        // In verbose mode, still show full token info
        ...
        // ---- Auto-save or per-turn prompt ----
        if (SessionContext.autoSave === true) {
            await saveChatHistory(messages, model_name);
        } else if (SessionContext.autoSave === false) {
            // No prompt, no save
        } else {
            // null = legacy: ask every turn
            const saveChoice = await ask(
                colorize("Do you want to save current session?\n1. Yes.\n2. No:\n", C.warning)
            );
            if (saveChoice.trim() !== "2") {
...
export async function runChat() {
    ...
    // Prompt for auto-save preference
