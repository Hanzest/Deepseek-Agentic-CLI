...
const PRICING = {
    "deepseek-v4-flash": { input: 0.0028, output: 0.28 },
    "deepseek-v4-pro":   { input: 0.003625, output: 0.87 },
};
    ...
    system_prompt:
        `
            ...
            **Plan First:** In Plan Mode, create a artifacts/temp-plan.md for the plan.
            ...
            modified, or deleted, with a one-line description of the change for each.
            - Task Delegation: A list of any sub-tasks that can be delegated to specialized
            agents or tools, along with the rationale for delegation. (Max 5 lines summary per sub-task)
            - Implementation Checklist: A checklist of the steps you will take to implement the plan, in order.
            This is your "to-do list" for Agent Mode. 
        `
};
...
function printSessionBanner(model_name, thinking) {
    ...
    const thinkingLabel = thinking?.thinking?.type === "enabled" ? "enabled" : "disabled";
...
function printStatus(model_name, messages, token_estimates, thinking) {
    ...
    // Estimated cost: (input/1M * inputRate) + (output/1M * outputRate)
