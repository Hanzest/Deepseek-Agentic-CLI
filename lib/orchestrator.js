...
const HYPERPARAMETERS = {
    ...
    system_prompt:
        `
            ...
            4. **Mode Toggle:** The user can type \`/plan\` or \`/agent\` at any time
            to switch modes. You remain in Plan Mode until the user explicitly
            ...
            in \`/docs\` and \`/docs/skills\` to reflect the changes.
        `
};
...
async function multiTurnLoop(model_name) {
    ...
    while (!stop) {
        ...
        const user_input = await ask(
            "Enter your message (type 'exit' to quit, /plan or /agent to switch modes):\n"
        );
        ...
        if (trimmed === "/plan") {
            ...
            console.clear();
            console.log(
                `\x1b[36m[Mode] Switched to Plan Mode — file mutation and system execution are now restricted (artifacts/ exempt).\x1b[0m`
            );
            continue;
        }
        if (trimmed === "/agent") {
            ...
            console.clear();
            console.log(
                `\x1b[32m[Mode] Switched to Agent Mode — all tools available.\x1b[0m`
            );
            continue;
        }
