...
import { saveChatHistory, saveAuditHistory, saveCondensedAudit, sanitizeFilename } from "./chatHistory.js";
import { condenseMessages } from "./contextCondenser.js";
import { condenseViaClone } from "./cloneCondenser.js";
...
const SessionContext = {
    ...
    outerCondensationApplied: false, // circuit-breaker for outer (turn-start) condensation
    innerLoopCondensationCount: 0,   // counter for inner-loop condensation attempts (max 1 per turn)
    condensationAuditTrail: [],        // [{ timestamp, originalCount, tokenReduction, originalTokens, newTokens }]
};
...
function getAuditData(model_name, token_estimates = null, messages = null) {
    ...
    if (token_estimates) {
        ...
        const est = estimateTokens(messages, "", 1.5);
    ...
    return {
        ...
        condensationEvents: (SessionContext.condensationAuditTrail || []).map(e => ({ ...e })),
        grandTotal: {
...
function printAudit(model_name, token_estimates = null) {
    ...
    // --- CONTEXT CONDENSATION AUDIT ---
    const condTrail = SessionContext.condensationAuditTrail || [];
    if (condTrail.length > 0) {
        console.log(colorize("  CONTEXT CONDENSATION EVENTS", C.heading));
        for (const entry of condTrail) {
            const reductionPct = entry.originalTokens > 0
                ? ((entry.tokenReduction / entry.originalTokens) * 100).toFixed(0)
                : "?";
            console.log(
                `  Turn ${entry.turnNumber}: ${entry.originalCount} msgs condensed. ` +
                `Tokens: ${entry.originalTokens} → ${entry.newTokens} ` +
                `(${colorize(`-${reductionPct}%`, C.success)})`
            );
        }
        console.log(`  Total condensation events: ${condTrail.length}`);
        console.log("");
    }
...
async function multiTurnLoop(model_name) {
    ...
    while (!stop) {
        ...
        // Optimization: Context Condensation Pipeline
        // Attempt semantic condensation first; fall back to sliding-window splice.
        let token_estimates = estimateTokens(
            messages,
            reasoning_history,
            HYPERPARAMETERS.token_multiplier
        );
        let condensationApplied = false;
        let condensationFailed = false;
        if (
            token_estimates.total_tokens > HYPERPARAMETERS.token_limit * 0.65 &&
            !SessionContext.outerCondensationApplied
        ) {
            // Deep-copy messages BEFORE condensation for audit trail
            const messagesBeforeCondense = JSON.parse(JSON.stringify(messages));

            console.log(colorize("[Context Condenser] Condensing...", C.system));
            // Primary: clone condenser — uses orchestrator's model + system prompt
            // for API cache-hit benefits on the shared prefix.
            let result = await condenseViaClone(
                messages,
                HYPERPARAMETERS.system_prompt,
                HYPERPARAMETERS.token_limit,
                client,
                model_name,
                { threshold: 0.65, preserveTailCount: 3 }
            );
            // Fallback: original deepseek-v4-flash condenser if clone path fails
            if (!result) {
                console.log(colorize("[Context Condenser] Clone path unavailable. Falling back to dedicated condenser.", C.dim));
                result = await condenseMessages(
                    messages,
                    HYPERPARAMETERS.token_limit,
                    client,
                    "deepseek-v4-flash",
                    { threshold: 0.65, maxRetries: 2, preserveTailCount: 3 }
                );
            }
            if (result) {
                // Replace messages with the condensed version
                ...
                SessionContext.outerCondensationApplied = true;

                // Record in session audit trail for /audit command visibility
                const auditEntry = {
                    timestamp: new Date().toISOString(),
                    originalCount: result.stats.originalCount,
                    tokenReduction: result.stats.tokenReduction,
                    originalTokens: result.stats.originalTokens,
                    newTokens: result.stats.newTokens,
                    turnNumber: SessionContext.turnCounter + 1,
                    mode: "clone", // indicates clone condenser was used
                };
                SessionContext.condensationAuditTrail.push(auditEntry);

                // Save before/after snapshots to disk with (CONDENSED AUDIT) prefix
                saveCondensedAudit(
                    messagesBeforeCondense,
                    messages,
                    model_name,
                ...
                console.log(
                    colorize(
                        `[Context Condenser] Condensed ${result.stats.originalCount} → 1 message. ` +
                        `Reduced by ~${result.stats.tokenReduction} tokens.`,
                        C.system
                    )
                );
            } else {
                condensationFailed = true;
            }
        }

        // Fallback: sliding-window splice (if condensation didn't run or failed)
        if (!condensationApplied) {
            if (condensationFailed) {
                console.log(
                    colorize("[Context Condenser] Semantic condensation failed. Falling back to sliding-window splice.", C.warning)
                );
            }
            let deleted_count = 0;
            while (
        ...
        // Reset the condensation circuit breakers for this new turn
        SessionContext.outerCondensationApplied = false;
        SessionContext.innerLoopCondensationCount = 0;
        ...
        for (let innerIter = 0; ; innerIter++) {
            ...
            // Mid-turn condensation check (re-triggers if tokens grew from tool calls)
            if (
                pre_call_tokens.total_tokens > HYPERPARAMETERS.token_limit * 0.65 &&
                SessionContext.innerLoopCondensationCount < 1
            ) {
                console.log(colorize("[Context Condenser] Mid-turn: Condensing...", C.system));
                // Primary: clone condenser (cache-hit optimized)
                let midResult = await condenseViaClone(
                    messages,
                    HYPERPARAMETERS.system_prompt,
                    HYPERPARAMETERS.token_limit,
                    client,
                    model_name,
                    { threshold: 0.65, preserveTailCount: 4 }
                );
                // Fallback: dedicated condenser if clone path fails
                if (!midResult) {
                    midResult = await condenseMessages(
                        messages,
                        HYPERPARAMETERS.token_limit,
                        client,
                        "deepseek-v4-flash",
                        { threshold: 0.65, maxRetries: 1, preserveTailCount: 4 }
                    );
                }
                if (midResult) {
                    messages = midResult.newMessages;
                    SessionContext.innerLoopCondensationCount++;
                    pre_call_tokens = estimateTokens(
                        messages,
                        reasoning_history,
                        HYPERPARAMETERS.token_multiplier
                    );
                    SessionContext.condensationAuditTrail.push({
                        timestamp: new Date().toISOString(),
                        originalCount: midResult.stats.originalCount,
                        tokenReduction: midResult.stats.tokenReduction,
                        originalTokens: midResult.stats.originalTokens,
                        newTokens: midResult.stats.newTokens,
                        turnNumber: SessionContext.turnCounter + 1,
                        location: "mid-turn",
                        mode: "clone",
                    });
                    console.log(
                        colorize(
                            `[Context Condenser] Mid-turn: condensed ${midResult.stats.originalCount} → 1 message. ` +
                            `Reduced by ~${midResult.stats.tokenReduction} tokens.`,
                            C.system
                        )
                    );
                } else {
                    console.log(
                        colorize("[Context Condenser] Mid-turn condensation failed.", C.warning)
                    );
                }
            }
            ...
            if (available_tokens <= 0) {
                // Attempt condensation at the brink before giving up
                if (SessionContext.innerLoopCondensationCount < 1) {
                    console.log(colorize("[Context Condenser] Brink: Condensing...", C.system));
                    // Primary: clone condenser (cache-hit optimized)
                    let brinkResult = await condenseViaClone(
                        messages,
                        HYPERPARAMETERS.system_prompt,
                        HYPERPARAMETERS.token_limit,
                        client,
                        model_name,
                        { threshold: 0.0, preserveTailCount: 2 }
                    );
                    // Fallback: dedicated condenser
                    if (!brinkResult) {
                        brinkResult = await condenseMessages(
                            messages,
                            HYPERPARAMETERS.token_limit,
                            client,
                            "deepseek-v4-flash",
                            { threshold: 0.0, maxRetries: 1, preserveTailCount: 2 }
                        );
                    }
                    if (brinkResult) {
                        messages = brinkResult.newMessages;
                        SessionContext.innerLoopCondensationCount++;
                        SessionContext.condensationAuditTrail.push({
                            timestamp: new Date().toISOString(),
                            originalCount: brinkResult.stats.originalCount,
                            tokenReduction: brinkResult.stats.tokenReduction,
                            originalTokens: brinkResult.stats.originalTokens,
                            newTokens: brinkResult.stats.newTokens,
                            turnNumber: SessionContext.turnCounter + 1,
                            location: "brink",
                            mode: "clone",
                        });
                        console.log(
                            colorize(
                                `[Context Condenser] Brink recovery: condensed ${brinkResult.stats.originalCount} → 1 message. ` +
                                `Reduced by ~${brinkResult.stats.tokenReduction} tokens.`,
                                C.success
                            )
                        );
                        continue; // Re-check at top of inner loop
                    }
                }
                console.log(
