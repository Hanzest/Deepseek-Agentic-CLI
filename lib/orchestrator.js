...
const HYPERPARAMETERS = {
    ...
    system_prompt:
        `
            ...
            ### Step 1: Context & Skill Gathering (Must Execute First)
            ...
            2. Scan \`docs/skills/\` for relevant SKILL.md files using these specific tool steps:
               a. **Discover** — Call \`get_project_tree("docs/skills", 2)\` to list all skill directories.
               b. **Match** — Call \`multi_file_search_string\` with \`glob_pattern: "docs/skills/**/*.md"\` 
                  and your topic keywords (or regex on the Tags: field) to find the most relevant skill(s).
               c. **Read** — Call \`read_file_chunk\` on the matched SKILL.md file(s) to extract domain 
                  constraints, principles, and anti-patterns.
               All three steps can be issued in a single batch call.
...
async function generateChatTitle(userMessage, modelName) {
    ...
    const cleaned = userMessage.replace(/^You are in (Plan|Agent) Mode\.\s*/i, "");
