...
export function ask(question) {
    ...
    return new Promise((resolve) => {
        rl.question(question, (answer) => {
            // Wait for the 'close' event before resolving so that
            // any subsequent readline interface on the same stdin
            // does not race with this one's cleanup (raw mode
            // restoration, listener removal, etc.).
...
export function createPromptLoop() {
    ...
    /**
     * Pause the persistent interface by closing it. The interface is
     * automatically recreated on the next askWithHistory() call.
     * Use this before running consent tool prompts (which create their
     * own temporary readline interfaces) to avoid double-echo of
     * keystrokes caused by two readline interfaces on the same stdin.
     */
    function pause() {
        if (rl) {
            return new Promise((resolve) => {
