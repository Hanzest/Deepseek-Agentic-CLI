...
export async function startChat() {
    ...
    console.log("  3. gemini-3-flash-preview (Gemini)");

    const model_choice = await ask(
        colorize("Enter your choice (1, 2, or 3): ", C.system)
    );
...
export async function thinkingToggle(provider = "deepseek") {
    if (provider === "gemini") {
        console.log(colorize("Choose thinking level (Gemini):", C.heading));
        console.log("  1. Minimal");
        console.log("  2. Low");
        console.log("  3. Medium");
        console.log("  4. High");

        const choice = await ask(
            colorize("Enter your choice (1-4): ", C.system)
        );
        ...
        const levels = { "1": "minimal", "2": "low", "3": "medium", "4": "high" };
        if (levels[choice]) {
            return { reasoning_effort: levels[choice] };
        }
        console.log(colorize("Invalid choice. Using minimal thinking by default.", C.warning));
