...
export function createPromptLoop() {
     ...
     */
    function close() {
        return _closeCurrentRL().then(() => {
            // Destroy stdin so the event loop drains and Node.js can exit
            // naturally on Windows. Without this, process.stdin.resume()
            // (called inside _closeCurrentRL) keeps stdin as an active
            // handle, preventing clean termination after /exit.
            if (process.stdin.isTTY) {
