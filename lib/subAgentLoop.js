...
function buildSystemPrompt(subAgentMarkdown, maxIterations) {
    return [
        ...
        "5. In your final response, you MUST append a JSON block inside a ```json ``` code block at the very end of your response with the following format:",
        "   ```json",
        "   {",
        "     \"files_modified\": [\"path/to/modified_file1.js\"],",
        "     \"files_created\": [\"path/to/new_file2.js\"],",
        "     \"verification_status\": \"passed\" | \"failed\" | \"partial\",",
        "     \"verification_details\": \"short details about verification\",",
        "     \"key_decisions\": [\"key decision description\"]",
        "   }",
        "   ```",
        "",
...
//                                    Defaults to console.
// ---------------------------------------------------------------------------
export async function runSubAgent(subAgentMarkdown, subAgentName, logger = console, agentMode = "agent", modelConfig = {}, toolsMap = {}, maxWallTimeSeconds = 300) {
    ...
    const MAX_WALL_TIME_MS = maxWallTimeSeconds * 1000;

    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        if (Date.now() - startTime > MAX_WALL_TIME_MS) {
            log(`\n\x1b[93m  ${tag} Max wall-clock time (${maxWallTimeSeconds}s) reached. Forcing termination.\x1b[0m`);
            return {
                status: "timeout",
                finalContent: "Task terminated: reached maximum wall-clock time limit.",
                reasoningContent: "",
        ...
        if ((iteration + 1) % 3 === 0) {
            console.log(`\x1b[90m  [Heartbeat] Sub-agent '${subAgentName}' - Iteration ${iteration + 1}/${MAX_ITERATIONS}...\x1b[0m`);
        }
        ...
        try {
            ...
            const { reasoning_content, content, tool_calls, usage } =
                await printStreamResponse(stream, extra_body, subAgentName)
