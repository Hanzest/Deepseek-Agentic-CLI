 ...
 * the two watched RAG layers (knowledge/ and workspace/).
 *
 * Features:
 ...
 *   - Incremental indexing: add/change -> vector store + BM25, unlink -> removal.
...
/**
 * Rebuild the BM25 index from the full set of current chunks. Used after large
 * batches to keep the sparse index consistent.
 */
  ...
  try {
    ...
    for (const [layer, layerRoot] of Object.entries(cfg.watched)) {
      ...
      for (const absPath of files) {
        ...
        if (prevHash === fileHash) continue; // unchanged
        await indexFile(absPath, { layer, layerRoot, rel }, { onStatus: ctx.onStatus, batch: true });
      }
    }
    await rebuildBm25();
    await saveHashes();
...
export async function start({ onStatus = null, embedderOverride = null, vectorStoreOverride = null, bm25Override = null } = {}) {
  ...
  deps.bm25 = bm25Override || new BM25Index();
  ...
  // Kick off the initial background scan.
  setImmediate(() => {
  ...
  try {
    ...
    const watchRoots = [cfg.watched.knowledge, cfg.watched.workspace].filter((r) => fs.existsSync(r));
