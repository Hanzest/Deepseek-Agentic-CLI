...
import { extractText as extractorsExtractText } from './extractors/index.js';
import { sanitizeText } from './extractors/sanitize.js';

const getHashesPath = () => path.join(projectRoot(), '.rag', 'hashes.json');
const getBm25Path = () => path.join(projectRoot(), '.rag', 'bm25.json');
const getVersionPath = () => path.join(projectRoot(), '.rag', 'index-version.json');
...
async function loadHashes() {
  try {
    const raw = await fs.promises.readFile(getHashesPath(), 'utf8');
...
/**
 * Read the persisted index schema version. Returns null when absent/corrupt.
 * @returns {Promise<number|null>}
 */
async function readIndexVersion() {
  try {
    const raw = await fs.promises.readFile(getVersionPath(), 'utf8');
    const parsed = JSON.parse(raw);
...
/**
 * Persist the current index schema version marker.
 * @returns {Promise<void>}
 */
async function writeIndexVersion() {
  try {
    const cfg = getConfig();
    const filePath = getVersionPath();
    await fs.promises.mkdir(path.dirname(filePath), { recursive: true });
    await fs.promises.writeFile(
      filePath,
      JSON.stringify({ version: Number(cfg.index?.schemaVersion) || 1 }, null, 2),
      'utf8'
    );
  } catch (err) {
    console.warn('[watcher] failed to persist index-version.json:', err.message);
  }
}
...
/**
 * Resolve which layer owns a given absolute path (multi-root aware).
 * @param {string} absPath - Absolute path.
 * @returns {{layer:string, layerRoot:string, rel:string}|null} Layer info or null.
 */
function resolveLayer(absPath) {
  const cfg = getConfig();
  for (const [layer, roots] of Object.entries(cfg.watched)) {
    const rootList = Array.isArray(roots) ? roots : [roots];
    for (const layerRoot of rootList) {
...
/**
 * Flatten every watched root across all layers (for scans / watch roots).
 * @returns {Array<{layer:string, layerRoot:string}>} Root entries.
 */
function allRoots() {
  const cfg = getConfig();
  const out = [];
...
/**
 * Extract raw text from a file, then strip invisible Unicode code points used
 * to hide document-borne prompt injection (zero-width spacers, bidi controls,
 * Unicode tag block). Format-specific extraction is delegated to
 * lib/rag/extractors/ (ported from book-to-skill); every extractor is
 * defensive and returns '' on failure, so this function never throws.
 *
 * @param {string} absPath - Absolute file path.
 * @param {string} ext - Lowercased file extension.
 * @returns {Promise<string>} Sanitized extracted text content.
 */
export async function extractText(absPath, ext) {
  const raw = await extractorsExtractText(absPath, ext);
...
async function handleUnlinkDir(dirPath) {
  ...
  if (!layer) return;
  if (isExcluded(dirPath, layer.layerRoot)) return;
...
async function initialScan(ctx) {
  runningBatches += 1;
  try {
    // Signal "indexing..." before the walk begins.
    await emitStatus(ctx.onStatus);
    for (const { layer, layerRoot } of allRoots()) {
...
/**
 * Synchronize all watched directories into the index: scan for added, modified,
 * or deleted files and update the vector store, BM25 index, and disk caches.
 * Executed at session end (/exit, /new) or when explicitly requested via /rag sync.
 * @param {object} [opts]
 * @param {Function|null} [opts.onStatus=null] - Status reporter.
 * @returns {Promise<{indexedCount: number, removedCount: number}>}
 */
export async function sync({ onStatus = null } = {}) {
  let indexedCount = 0;
  let removedCount = 0;
  runningBatches += 1;
  try {
    await loadHashes();
    const liveAbs = new Set();
    let changed = false;
    ...
    for (const { layer, layerRoot } of allRoots()) {
      ...
      for (const absPath of files) {
        ...
        const rel = relKey(absPath, layerRoot);
        if (rel === null) continue;
        const indexed = await indexFile(absPath, { layer, layerRoot, rel }, { onStatus, batch: true });
        if (indexed) {
          indexedCount += 1;
          changed = true;
          await new Promise((r) => setImmediate(r));
    ...
    // Clean removed / orphaned files
    for (const absPath of [...hashes.keys()]) {
      if (!liveAbs.has(absPath)) {
        const layer = resolveLayer(absPath);
        if (layer) {
          try {
            await deps.vectorStore.removeByFile(absPath);
          } catch (err) {
            console.warn(`[watcher] sync remove failed for ${absPath}:`, err.message);
          }
          removeBm25ForFile(layer.layer, layer.rel);
        }
    ...
    if (changed || !fs.existsSync(getBm25Path()) || !fs.existsSync(getHashesPath())) {
      await rebuildBm25();
      await saveHashes();
      await writeIndexVersion();
    }
    ...
    console.warn('[watcher] sync failed:', err.message);
  } finally {
    runningBatches -= 1;
    await refreshChunkCount();
  }
...
/**
 * Start the watcher: init stores, check/restore the cache, and optionally watch.
 *
...
export async function start({ onStatus = null, embedderOverride = null, vectorStoreOverride = null, bm25Override = null } = {}) {
  ...
  // Cache-aware startup: when the index schema-version marker matches AND both
  // the BM25 search cache and the hash cache exist, restore the shared index
  // from disk and SKIP the full directory re-scan so the CLI boots instantly
  // (no LanceDB scan / re-tokenization). A version mismatch (config/layout
  // change) invalidates both caches so the next scan rebuilds them consistently.
  const versionOk = (await readIndexVersion()) === (Number(getConfig().index?.schemaVersion) || 1);
  const bm25File = getBm25Path();
  const hashesFile = getHashesPath();
  let cacheReady = versionOk && fs.existsSync(bm25File) && fs.existsSync(hashesFile);
  if (!versionOk) {
    // Stale caches from a previous index layout — drop them and rescan.
    try { fs.rmSync(bm25File, { force: true }); } catch { /* ignore */ }
    hashes = new Map();
    await saveHashes();
    cacheReady = false;
  }
  ...
  if (cacheReady && deps.bm25 && deps.bm25.totalDocs > 0) {
    ...
    // Kick off the initial background scan only on first run / missing cache.
    setImmediate(() => {
      initialScan(ctx).catch((err) => console.warn('[watcher] background scan error:', err.message));
    });
  }

  // Set up the chokidar file watcher if active_layers is non-empty.
  // By default, active_layers is empty (event-driven sync at session end).
  const activeLayers = Array.isArray(cfg.watcher?.active_layers)
    ? cfg.watcher.active_layers
    : [];
  const watchRoots = activeLayers
    .flatMap((layerName) => {
      const roots = cfg.watched[layerName];
  ...
  if (watchRoots.length > 0) {
    try {
      ...
      watcher = chokidar.watch(watchRoots, {
        ignored: (absPath) => {
          const layer = resolveLayer(absPath);
          if (!layer) return !watchRoots.some((r) => r.startsWith(absPath) || absPath.startsWith(r));
          return isExcluded(absPath, layer.layerRoot);
        },
        ignoreInitial: true,
...
export async function forceReindex() {
  ...
  hashes = new Map();
  await saveHashes();
  for (const { layerRoot } of allRoots()) {
    if (!fs.existsSync(layerRoot)) continue;
    try {
      const files = await walk(layerRoot, layerRoot);
      for (const absPath of files) {
...
export async function cleanOrphans() {
  // Hash cache is keyed by absolute path — collect the live set across roots.
  const liveAbs = new Set();
  for (const { layerRoot } of allRoots()) {
    if (!fs.existsSync(layerRoot)) continue;
    const files = await walk(layerRoot, layerRoot);
    for (const absPath of files) liveAbs.add(absPath);
  }

  for (const absPath of [...hashes.keys()]) {
    if (liveAbs.has(absPath)) continue;
    const layer = resolveLayer(absPath);
    if (layer) {
      try {
        await deps.vectorStore.removeByFile(absPath);
      } catch (err) {
        console.warn(`[watcher] clean orphan remove failed for ${absPath}:`, err.message);
      }
      removeBm25ForFile(layer.layer, layer.rel);
    }
    hashes.delete(absPath);
...
export default { start, stop, sync, forceReindex, cleanOrphans };
