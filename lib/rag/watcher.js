...
/** Marker file recording the index schemaVersion that produced the caches. */
const VERSION_PATH = path.join(ROOT, '.rag', 'index-version.json');
...
async function readIndexVersion() {
  try {
    const raw = await fs.promises.readFile(VERSION_PATH, 'utf8');
    const parsed = JSON.parse(raw);
...
async function writeIndexVersion() {
  try {
    const cfg = getConfig();
    await fs.promises.mkdir(path.dirname(VERSION_PATH), { recursive: true });
  ...
  try {
    ...
    for (const { layer, layerRoot } of allRoots()) {
      ...
      for (const absPath of files) {
        ...
        // indexFile performs its own hash + skip-if-unchanged logic.
...
export async function start({ onStatus = null, embedderOverride = null, vectorStoreOverride = null, bm25Override = null } = {}) {
  ...
  let cacheReady = versionOk && fs.existsSync(BM25_PATH) && fs.existsSync(HASHES_PATH);
  if (!versionOk) {
    // Stale caches from a previous index layout — drop them and rescan.
    try { fs.rmSync(BM25_PATH, { force: true }); } catch { /* ignore */ }
    hashes = new Map();
    await saveHashes();
    cacheReady = false;
  }
  ...
  // Set up the chokidar file watcher — LIVE watching covers the active layers:
  // knowledge/ (permanent reference + skills) and workspace/ (the live project).
  try {
