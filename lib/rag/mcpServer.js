...
const RAG_SEARCH_TOOL = Object.freeze({
  name: 'rag_search',
  description: 'Hybrid RAG retrieval over the indexed project + knowledge base',
  inputSchema: {
    type: 'object',
    properties: {
      query: { type: 'string' },
      ...
      search_mode: { type: 'string', enum: ['hybrid', 'keyword', 'dense'], default: 'hybrid' },
    },
    required: ['query'],
  },
});
...
async function callRagSearch(id, arguments_) {
  ...
  const { query, namespace, layer, top_k, min_score, max_prompt_tokens, search_mode } = arguments_;
  ...
  try {
    ...
    const output = await search({ query, namespace, layer, top_k, min_score, max_prompt_tokens, search_mode });
