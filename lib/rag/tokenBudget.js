...
import { hasCJK, estimateTokensCJK } from './extractors/tokens.js';
...
export function countTokens(text, tokenizerFn = null) {
  ...
  // them by orders of magnitude; count CJK code points directly (port of
  // book-to-skill's estimate_tokens). Non-CJK keeps the ~4 chars/token heuristic.
  if (hasCJK(text)) {
