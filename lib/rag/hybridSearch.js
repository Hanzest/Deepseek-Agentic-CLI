...
let bm25Path = path.join(projectRoot(), '.rag', 'bm25.json');
...
export function setBm25Path(filePath) {
  bm25Path = filePath === null || filePath === undefined ? null : String(filePath);
}
...
async function loadSearchCache() {
  if (!bm25Path) return null;
  try {
    const raw = await fs.promises.readFile(bm25Path, 'utf8');
...
export async function saveBm25Index(index = null) {
  ...
  try {
    ...
    await fs.promises.mkdir(path.dirname(bm25Path), { recursive: true });
    const tmp = `${bm25Path}.tmp`;
    await fs.promises.writeFile(tmp, JSON.stringify(payload), 'utf8');
    await fs.promises.rename(tmp, bm25Path);
    return true;
  } catch (err) {
