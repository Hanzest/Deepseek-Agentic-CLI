...
import { rebuildIndex, saveBm25Index } from './hybridSearch.js';
...
export async function init({ onStatus = null } = {}) {
  try {
    ...
    // Lazy, non-fatal: load the FlashRank reranker so the enabled-by-default
    // rerank path in hybridSearch actually has a ready model. Never blocks the
    // watcher startup when the model cache is not yet warmed.
    await reranker.init({ threads: config.watcher?.cpu_threads ?? null });
    await rebuildIndex();
...
export async function search({
  ...
  try {
    ...
    if (max_prompt_tokens == null && typeof budgetEstimator === 'function') {
      try {
        const estimate = budgetEstimator({ query, namespace, layer: resolvedLayer, top_k, filters, search_mode });
...
export async function getStatus() {
  try {
    ...
    const config = getConfig();
    const watchedPaths = Object.values(config.watched ?? {})
      .flat()
      .filter(Boolean);
...
/**
 * Synchronize the RAG index with the current file state on disk.
 * @param {object} [opts]
 * @param {Function|null} [opts.onStatus=null]
 * @returns {Promise<{indexedCount: number, removedCount: number}>}
 */
export async function sync({ onStatus = null } = {}) {
  try {
    return await watcher.sync({ onStatus });
  } catch (err) {
    console.warn('[rag] sync failed:', err?.message ?? err);
    return { indexedCount: 0, removedCount: 0 };
  }
}

/**
 * Shut down the RAG runtime: synchronize changed files, stop the watcher and close the vector store.
 * @param {object} [opts]
 * @param {boolean} [opts.sync=true] Whether to run an incremental sync of changed files on shutdown.
 * @returns {Promise<void>}
 */
export async function shutdown({ sync: shouldSync = true } = {}) {
  if (shouldSync) {
    try {
      await watcher.sync();
    } catch (err) {
      console.warn('[rag] shutdown sync failed:', err?.message ?? err);
    }
  }
  try {
    await watcher.stop();
  ...
  try {
    ...
    console.warn('[rag] shutdown (save bm25 cache) failed:', err?.message ?? err);
  }
  try {
