...
export async function saveChatHistory(messages, modelName, title) {
    ...
    const dirPath = path.join(REPO_ROOT, "chat_history", dateFolder);
...
/**
 * Recursively scans a directory for non-audit JSON chat history files.
 *
 * @param {string} dir - The directory path to scan.
 * @returns {Array<string>} List of absolute file paths.
 */
export function findChatFiles(dir) {
    let results = [];
    ...
    try {
        ...
        for (const file of list) {
            ...
            if (stat && stat.isDirectory()) {
                results = results.concat(findChatFiles(fullPath));
            } else {
